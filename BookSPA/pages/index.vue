<template lang="html">
	<div class="bg-[#F7F8FA] pb-4 px-5 xs:px-7 sm:px-[52px]">
		<SearchAndAddComponent
			:total-books="savedBooks.length"
			v-model="search"
			@update:open-add-book-form="isOpenAddBookForm = $event"
			@sort-by="sortBy($event, savedBooks)"
		/>
	</div>
	<div
		class="flex flex-col flex-grow gap-4 py-4 px-5 h-min overflow-scroll not-scrollbar xs:px-7 sm:px-[52px]"
		v-if="searchedResults !== false"
	>
		<div
	...
	<teleport
		...
		v-if="mounted && Object.keys(notification).length > 0"
	>
		<button
